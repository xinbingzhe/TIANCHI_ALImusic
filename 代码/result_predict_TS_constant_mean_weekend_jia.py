...
def polynomial_test(x_train,y_train,x_test,y_test):
     degree = [1,2,3]
     result = {}
     rmse_list = []
     for d in degree:      
...
def mean_value(y_train,degree):
    ...
    y_predict = temp
    return y_predict
def diZeng_value(start,jia):
    degree=int(jia)
    start=int(start)
    print("递增 %d %d"%(start,degree))
    temp=[]
    for i in range(0,60):
        start+=degree
        temp.append(start)
    y_predict = temp
    return y_predict
def diJian_value(start,jian):
    degree=int(jian)
    start=int(start)
    temp=[]
    print("递减 %d %d"%(start,degree))
    for i in range(0,60):
        start-=degree
        temp.append(start)
    y_predict = temp
...
fr2 = open('E:/tianchi/season2(1)/p2/data/model/train_Model1_withoutp.txt','r')
fw = open('E:/tianchi/season2(1)/p2/data/result/predict_train_model_with_p1_jia.csv','w')
line1 = fr1.readline()
line2 = fr2.readline()
inde = 0
...
while line1:
    ...
    if model == 'p':
        ...
        y_predict=diJian_value(degree,jian)
        plt.scatter(x_train,y_train)
        plt.plot(x_test,y_predict,color='red')
    elif model=='jia':
        y_predict=diZeng_value(degree,jia)
        plt.scatter(x_train,y_train)
        plt.plot(x_test,y_predict,color='red')
    else :
        print('failed')
    ...
    plt.grid()
    path = "E:/tianchi/season2(1)/p2/result_train_p1_jia_fig/"+name.strip('\n')
    #print(path)
